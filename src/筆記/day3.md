...
當使用者點擊一級目錄、二級目錄、三級目錄時，由Home模組跳轉到Search模組，將使用者選中的產品分類(產品分類名稱、產品分類Id)在路由跳轉時進行參數的傳遞。
...
三級連動: 若使用宣告式導航router-link，可以實現路由跳轉傳遞參數。但須需要注意，會出現卡頓現象。

+ router-link: 是一個元件，當server返回數據之後，需要循環創建很多router-link元件 (創建VueComponent instance) 1000+，創建元件實例時，一瞬間創建1000+會佔據大量的記憶體資源，因此出現了卡頓的現象。 
